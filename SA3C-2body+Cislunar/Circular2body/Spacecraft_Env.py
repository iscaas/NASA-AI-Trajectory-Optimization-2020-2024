...
class Spacecraft_Env(gym.Env):
  ...
  def __init__(self, args):
    ...
    self.max_steps_one_ep = chosen_case['max_steps_in_ep'][0]
    ...
    if self.testing_weights== 0 or args.single_weight_test == 1:
      ...
      folders = ['h', 'hxy', 'exy', 'a', 'ecc', 'inc', 'mass', 'phi', 'sum_reward', 'time', 'Successful_episodes', 'thrust']
      ...
      self.folder_path_h = os.path.join(f"{folder_paths[0]}/{'h'}")
      self.folder_path_hxy = os.path.join(f"{folder_paths[1]}/{'hxy'}")
      self.folder_path_exy = os.path.join(f"{folder_paths[2]}/{'exy'}")
      self.folder_path_a = os.path.join(f"{folder_paths[3]}/{'a'}")
      self.folder_path_ecc = os.path.join(f"{folder_paths[4]}/{'ecc'}")
      self.folder_path_inc = os.path.join(f"{folder_paths[5]}/{'inc'}")
      self.folder_path_mass = os.path.join(f"{folder_paths[6]}/{'mass'}")
      self.folder_path_phi = os.path.join(f"{folder_paths[7]}/{'phi'}")
      self.folder_path_sum_reward = os.path.join(f"{folder_paths[8]}/{'reward'}") 
      self.folder_path_time = os.path.join(f"{folder_paths[9]}/{'ep_time'}")
      self.successful_episodes = os.path.join(f"{folder_paths[10]}")
      self.folder_path_thrust = os.path.join(f"{folder_paths[11]}/{'thrust'}")
  ...
  def reset(self):
    ...
    if args.testing_weights == 0 or args.single_weight_test == 1:
      if self.ep_counter < 1:
  ...
  def step(self, action):
    ...
    if self.testing_weights== 0 or args.single_weight_test == 1:
      ...
      if ((redflag==1) or (self.ep_Cont_len_counter % 2500 == 0) or (self.ep_fixed_len_counter > self.max_steps_one_ep )):
        ...
        self.env.plot_two_variable("hx_hy","hx","hy", self.hx_history,self.hy_history,  self.folder_path_hxy, self.ep_counter, hx_hy_flag=1, tolhx= 500, tolhy= 500)
        self.env.plot_two_variable("ex_ey","ex","ey", self.ex_history,self.ey_history,  self.folder_path_exy, self.ep_counter, ex_ey_flag=1, tolex= 0.1, toley= 0.1)
        self.env.plot_variable("Time(days)", self.time_in_days_history, self.folder_path_time, self.ep_counter)
        ...
        self.env.plot_variable("Thrust", self.thrust_history, self.folder_path_thrust, self.ep_counter)
    ...
    if self.done:
      ...
      if self.testing_weights== 0  or args.single_weight_test == 1:
        ...
        self.env.plot_two_variable("hx_hy","hx","hy", self.hx_history,self.hy_history,  self.Successful_Episode_num, self.ep_counter, flag_saving_with_no_ep_nu =1, hx_hy_flag=1, tolhx= 500, tolhy= 500)
        self.env.plot_two_variable("ex_ey","ex","ey", self.ex_history,self.ey_history,  self.Successful_Episode_num, self.ep_counter, flag_saving_with_no_ep_nu =1, ex_ey_flag=1, tolex= 0.1, toley= 0.1)
    ...
    if self.testing_weights== 0 or args.single_weight_test == 1:
      ...
      print(self.counter,"/", self.total_timesteps," Sh:",self.sh_command_nu, "  Ep_nu :", self.ep_counter , "  Ep_len :", self.ep_fixed_len_counter , 
            "  step_reward:" , self.reward[0] ,   "  Acc_reward: ", self.acc_reward,
