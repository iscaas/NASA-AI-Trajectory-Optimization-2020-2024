...
class Enviornment:
    def __init__(self, eng, args):
        ...
        self.target_state = np.array([29640.229, 0, 0, 0, 0, 0, 0, 0])
        ...
        if not os.path.exists(self.completeName_csvlist):
            with open(self.completeName_csvlist, 'w') as f:
                 pass
        if not os.path.exists(self.completeName_csvlistinitialize):
            with open(self.completeName_csvlistinitialize, 'w') as f:
                pass
        ################################################################################################ 
    ...
    def is_terminal(self,state_5):
        ...
        self.flag_a = flag_for_error_a
             
        return flag, ecc, self.tol_ecc, a, self.tol_a_low, self.tol_a_high, i,self.tol_inc  , 0, self.flag_a, 0,monitor_a_flag, red_flag
    ...
    def get_seg_value(self, state_5):    
        ...
        if args.case in ['10']:
            tol_inc = 0.1#0.1;                  # tolerance of inclination +- deg    0.1  0.01
            tol_ecc = 0.1#0.01;                 # 0.00001 tolerance of eccentricity +0  0.01  0.00001
            tol_a_2 =  100#200              #  300, 30   
        else:
            tol_inc = 0.1;                  # tolerance of inclination +- deg    0.1  0.01
            tol_ecc = 0.01;                 # 0.00001 tolerance of eccentricity +0  0.01  0.00001
            tol_a_2 = 200              #  300, 30  
    ...
    def step( self, state, alpha, beta, F, segment, m01, I_sp, Max_R_time , phi_normalizing_factor,   timestep):
        ...
        F_temp = F
        self.eclipse_flag =  self.eng.Mat_env.chkEclipse(self.completeName_csvlist) # if timestep >=0 else 0
        if self.eclipse_flag * self.shadow_flag:
            F = 0
        # if timestep > 5000:
        #      F = F_temp
        #      segment = 0.1*(3.14/180) 
        ...
        try:
            a1 =  self.eng.Mat_env.resulting(self.completeName_csvlist)
            ...
            done,ecc, tol_ecc, a, tol_a_low, tol_a_high,  i,tol_inc, self.flag_ecc, self.flag_a, self.flag_inc, monitor_a_flag,redflag   =self.is_terminal(t_state[0:5])
            target_state_parameters = [ecc,a, i , tol_ecc, tol_a_low, tol_a_high, tol_inc,self.flag_ecc, self.flag_a, self.flag_inc,  F ]
        ...
        if (self.flag_raise_error==1): 
            ...
            target_state_parameters = [ecc,a, i , tol_ecc, tol_a_low, tol_a_high, tol_inc,self.flag_ecc, self.flag_a, self.flag_inc  ]
            reward,  distance, dist_aei, ecc_new, a_new, i_new  = np.array([-100 ]), 0, 0, ecc, a, i


        return t_state , reward, distance, dist_aei, 1==done , 1, target_state_parameters, redflag, time_in_days , ecc_new, a_new, i_new, self.eclipse_flag* self.shadow_flag , self.segment_flag, self.time_before_seg_change_1, self.time_after_seg_change, segment
